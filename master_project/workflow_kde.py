...
def location_based_sampling(classifier, X_pool):
    ...
    overall_dist_sorted = sorted(overall_distances, reverse=True)
    for _dist in overall_dist_sorted:
        _query_idx = overall_distances.index(_dist)
        if _query_idx in xs_idx:
            continue
    return [_query_idx], X_pool[_query_idx]
...
while i <= n_queries:
    ...
    if i % 50 == 0 and i > 0:
        ...
        if i == 650:
            model_ae.save(f"models/model_ae_al2_{mode}_650_{CLUSTER_NUM}cluster.h5")
            print("saving predictions...")
            preds = model_ae.predict(x_test_ae_cnn)
            result = pd.DataFrame(preds, columns=["x", "y", "z"])
            result.to_csv(f"preds/cnn_ae_al2_{mode}_{i}_{CLUSTER_NUM}cluster.csv")
            best_mse = mse
            exit(0)
